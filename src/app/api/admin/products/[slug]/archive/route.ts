// POST /api/admin/products/[slug]/archive — soft delete (active=false).

import { NextRequest } from "next/server";
import { revalidatePath } from "next/cache";
import { getLaceDb } from "@/lib/db";
import { archiveProductBySlug } from "@/lib/agent/actions";
...
export async function POST(req: NextRequest, { params }: RouteParams) {
  ...
  if (actor.role !== "owner") {
    return fail("Archive needs an owner.", { status: 403 });
  }
  ...
  const result = await archiveProductBySlug(db, slug, {
    actorLabel: actorLabel(actor),
    actorType: "user",
  });
  if (!result.ok)
    return fail(result.error ?? "Could not archive the product.", {
      status: 400,
    });
